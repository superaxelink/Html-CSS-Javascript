...
export class Rolling extends State{
  ...
  handleInput(input){
    ...
    //check button is pressed to keep rolling
    //unshift adds one or more elements to the beginning of the array and returns the new length of the array
    if( (!input.includes('Enter') && !input.includes(' ')) && this.game.player.onGround() ){
      this.game.player.setState(states.RUNNING, 1);
    } else if( (!input.includes('Enter') && !input.includes(' ')) && !this.game.player.onGround() ){
      this.game.player.setState(states.FALLING, 1);
    } else if((input.includes('Enter') || input.includes(' ')) && input.includes('ArrowUp') && this.game.player.onGround()){
