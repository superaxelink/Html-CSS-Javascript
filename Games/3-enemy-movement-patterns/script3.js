//Horizontal and vertical senoidal movement
...
class Enemy{
	constructor(){
		...
		this.spriteWidth = 218; //this must be according to the size of each png image
...
//The interval between 3 and 4 is abitrary.
		this.angle = Math.random()*500; // Indicates where in the senoidal movement the position begins. So we initialize position of enemies at random.
		this.angleSpeed = Math.random()*0.5 + 0.5; //randomize movement of each enemy
		this.curve = Math.random()*200 + 50; //changes amplitued of senoidal movement
	}
	update(){ //To update movement
		this.x = this.curve * Math.cos(this.angle * Math.PI/90) + (canvas.width/2 - this.width/2); //this seems similar to brownian movement
//producing an oscillatory horizontal movement 
